...
def nrt_forecast(idate):
    ...
    ocean = ocean.resample({'leadtime':'h'}).interpolate()
    ...
    winds = winds.resample({'leadtime':'h'}).interpolate()  
    ...
    # ---------------------------------------------------------------------------- #
    print('Plotting...')
    fig, ax, cax =  make_maps((1,4), figsize=(20,10),
                              extent=extent)
    cax.axis('off')
    vmin,vmax=0, 15
    cmap='viridis'
    ...
    vmin,vmax=0,50
    cmap='Blues'
    m=ax[0,1].contourf(ocean.lon, ocean.lat, fill_borders(cs)*100,
                  cmap=cmap, extend='both', levels=np.arange(vmin,
                                                             vmax+0.5,
                                                             0.5))
    # ax[0,1].streamplot(ocean.lon,ocean.lat, ocean.uo.values, ocean.vo.values,
    #                    density=2.0,
    #             transform=ccrs.PlateCarree(), color='k', linewidth=0.5,
    #             arrowsize=0.75)
    
    ax[0,1].quiver(ocean.lon,ocean.lat, ocean.uo.values, ocean.vo.values, scale=10,
                   width=0.003, transform=ccrs.PlateCarree(),
                   regrid_shape=30, alpha=0.5)
    
    fig.colorbar(m, ax=ax[0,1], orientation='horizontal', pad=0.01,
                 ticks=np.arange(vmin,vmax+10,10),
                 label='Velocidad de la corriente (m/s)')
    
    vmin,vmax = sst.min()-sst.min()%0.5+0.5,sst.max()-sst.max()%0.5-0.5
    cmap = cmocean.cm.thermal
    m=ax[0,2].contourf(sst.lon, sst.lat, fill_borders(sst),
                  cmap=cmap, extend='both', levels=np.arange(vmin,
                                                             vmax+0.25,
                                                             0.25))
    ...
    cmap = mcolors.ListedColormap(plt.cm.nipy_spectral(np.linspace(0.1,0.95,
                                                                   1000)))
    m=ax[0,3].contourf(waves.lon, waves.lat, fill_borders(waves[waveheight_name]),
    ...
    ax[0,3].quiver(waves.lon,waves.lat,u.values,v.values, scale=25,width=0.003,
              transform=ccrs.PlateCarree(), regrid_shape=25, alpha=0.35)
    fig.colorbar(m, ax=ax[0,3], orientation='horizontal', pad=0.01,
                 label='Altura significativa de ola (m)')
    ax[0,1].text(1.1, 1.17,
    ...
    ax[0,1].text(1.1, 1.05,
                 text,
                 transform=ax[0,1].transAxes,
                 ha='center', fontsize=15)
    ax[0,1].text(1.1, 1.01,
